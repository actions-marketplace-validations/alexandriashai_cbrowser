...
import { BUILTIN_PERSONAS, loadCustomPersonas, saveCustomPersona, deleteCustomPersona, isBuiltinPersona, generatePersonaFromDescription, listAccessibilityPersonas, } from "./personas.js";
...
PERFORMANCE REGRESSION (v6.4.0)
  ...
  perf-regression <url> <baseline>  Compare current performance against baseline
    --sensitivity <level>     strict|normal|ci|lenient (default: normal)
    ...
    try {
        switch (command) {
            ...
            case "smart-click": {
                ...
                const maxRetries = options["max-retries"] ? parseInt(options["max-retries"]) : 3;
                const retryDelay = options["retry-delay"] ? parseInt(options["retry-delay"]) : 1000;
            ...
            case "dismiss-overlay": {
                ...
                const overlayType = options.type || "auto";
                ...
                if (result.overlaysFound === 0) {
                    ...
                    for (const d of result.details) {
                        ...
                        if (d.closeMethod)
                            console.log(`      Method: ${d.closeMethod}`);
                        if (d.error)
            ...
            case "heal": {
                ...
                switch (subcommand) {
                    ...
                    case "test": {
                        ...
                        const cached = entries.find(e => e.originalSelector.toLowerCase() === selector.toLowerCase());
                        if (cached) {
            ...
            case "parallel": {
                ...
                switch (subcommand) {
                    case "devices": {
                        ...
                        const results = await CBrowser.parallelDevices(deviceList, async (b, _device) => {
            ...
            case "hunt": {
                ...
                const maxPages = options["max-pages"] ? parseInt(options["max-pages"]) : 10;
                const timeout = options.timeout ? parseInt(options.timeout) : 60000;
                console.log(`\n🔍 Bug Hunter starting...`);
                console.log(`   URL: ${url}`);
                ...
                if (result.bugs.length > 0) {
                    ...
                    for (const bug of result.bugs) {
                        ...
                        if (bug.selector)
                            console.log(`      Selector: ${bug.selector}`);
            ...
            case "chaos": {
                ...
                const chaosConfig = {};
                if (options.latency) {
                    chaosConfig.networkLatency = parseInt(options.latency);
                }
                if (options.offline) {
                    chaosConfig.offline = true;
                }
                if (options.block) {
                    chaosConfig.blockUrls = options.block.split(",");
                }
                if (options["fail-api"]) {
                    const [pattern, status] = options["fail-api"].split(":");
            ...
            case "flaky-check": {
                ...
                const flakyOptions = {
                    headless,
                    runs: options.runs ? parseInt(options.runs) : 5,
            ...
            case "perf-regression": {
                ...
                if (!url || !baselineName) {
                    ...
                    console.error("");
                    console.error("Options:");
                    console.error("  --sensitivity <level> strict|normal|ci|lenient (default: normal)");
                    ...
                    console.error("  ci:      FCP 25%/150ms, LCP 25%/300ms, TTFB 25%/75ms, CLS 25%/0.08 (for CI/CD)");
            ...
            case "agent-ready-audit": {
                ...
                const geoRegion = options["geo-region"];
                let auditProxy;
                if (geoRegion) {
                ...
                if (options.html) {
                    ...
                    const htmlPath = options.output?.replace(".json", ".html") || "agent-ready-report.html";
                    fs.writeFileSync(htmlPath, htmlReport);
    ...
    Generated by <a href="https://cbrowser.ai">CBrowser</a> v${(await import("./version.js")).VERSION}
            ...
            case "webmcp-ready": {
                ...
                const result = await runWebMCPReadyAudit(url, {
                    ...
                    oauthToken: options["oauth-token"],
                ...
                if (options.html) {
                    const fs = await import("fs");
                    const htmlPath = options.output?.replace(".json", ".html") || "webmcp-ready-report.html";
            ...
            case "competitive-benchmark": {
                ...
                const personaName = options.persona || "first-timer";
                const maxSteps = options["max-steps"] ? parseInt(options["max-steps"]) : 30;
                const maxTime = options["max-time"] ? parseInt(options["max-time"]) : 180;
                ...
                if (options.html) {
                    ...
                    const htmlPath = options.output?.replace(".json", ".html") || "competitive-benchmark-report.html";
                    fs.writeFileSync(htmlPath, htmlReport);
                    console.log(`🌐 HTML report saved: ${htmlPath}`);
                }
                break;
            ...
            case "empathy-audit": {
                const url = args[0];
                const goal = options.goal;
                const disabilitiesArg = options.disabilities;
                if (!url) {
                    console.error("Error: URL required");
                ...
                const wcagLevel = options["wcag-level"] || "AA";
                const maxSteps = options["max-steps"] ? parseInt(options["max-steps"]) : 20;
                const maxTime = options["max-time"] ? parseInt(options["max-time"]) : 120;
                ...
                if (options.html) {
                    ...
                    const htmlPath = options.output?.replace(".json", ".html") || "empathy-audit-report.html";
                    fs.writeFileSync(htmlPath, htmlReport);
                    console.log(`🌐 HTML report saved: ${htmlPath}`);
                }
