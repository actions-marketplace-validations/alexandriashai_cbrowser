...
function computeMotorFitness(data) {
    ...
    if (data.touchTargets && data.touchTargets.length > 0) {
        ...
        hitProbability = {
            score: 50,
            details: "No touch target size data available",
        };
    }
    // --- interactionCount (20%) ---
...
function computeCognitiveFitness(data) {
    ...
    // --- readability (20%) ---
    const flesch = safe(data.fleschScore, 60);
    const readabilityScore = clamp(Math.round(Math.min(100, flesch * 1.2)));
    const readability = {
        score: readabilityScore,
        details: `Flesch readability score: ${Math.round(flesch)} (higher = more readable)`,
    };
    ...
    // --- attentionClarity (20%) ---
    const captureRate = safe(data.ctaCaptureRate, 0.25);
    const attentionScore = clamp(Math.round(Math.min(100, captureRate * 200)));
    const attentionClarity = {
        score: attentionScore,
        details: `CTA capture rate: ${(captureRate * 100).toFixed(1)}% (higher = clearer attention guidance)`,
    };
    // --- predictability (15%) ---
