    ...
    for (const path of searchPaths) {
        if (existsSync(path)) {
            try {
                ...
                console.debug(`[CBrowser] Invalid config file ${path}: ${e.message}`);
            }
        }
    }
    return null;
...
export function getDefaultConfig() {
    ...
    return {
        ...
        colorScheme: process.env.CBROWSER_COLOR_SCHEME || configFile?.colorScheme,
 ...
 */
export function getScreenshotRetention() {
 ...
 */
export function cleanupOldScreenshotSessions(paths) {
    const p = paths || getPaths();
    ...
    if (retention === 0)
        return 0;
    if (!existsSync(p.screenshotsBaseDir))
        return 0;
    try {
 ...
 */
export function cleanupCurrentSessionScreenshots(paths) {
    const p = paths || getPaths();
    let deleted = 0;
    if (!existsSync(p.screenshotsDir))
        return 0;
    try {
 ...
 */
function countScreenshotSessions(baseDir) {
 ...
 */
export async function getStatusInfo(version, toolCount) {
    ...
    const directories = dirEntries.map(([name, dirPath, ext]) => ({
        name,
        path: dirPath,
    ...
    let healCache = { totalEntries: 0, totalHeals: 0, topDomain: null };
    if (existsSync(cachePath)) {
        try {
            ...
            console.debug(`[CBrowser] Corrupted selector cache: ${e.message}`);
        }
    }
    // Browser detection
