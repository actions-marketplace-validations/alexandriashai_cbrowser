 ...
 */
export function isBrowserNotInstalledError(error) {
 ...
 */
export async function installPlaywrightBrowsers(browserName) {
 ...
 */
export function getBrowserInstallErrorMessage(browserName = "chromium") {
    return `
╔══════════════════════════════════════════════════════════════════════════════╗
 ...
 */
export async function launchBrowserWithFallback(browserType = chromium, options = {}) {
        ...
        if (isBrowserNotInstalledError(error)) {
            ...
            if (installed) {
                ...
                try {
                    ...
                    throw new Error(`Browser launch failed after installation. ` +
...
export class CBrowser {
    ...
    saveSessionState(url, viewport, device) {
        try {
    ...
    saveDeviceSetting(device) {
        try {
            ...
            console.error(`Failed to save device setting: ${e.message}`);
        }
    }
    loadSessionState() {
        try {
     ...
     * Get actionable error message for browser installation failure.
     */
    getBrowserInstallErrorMessage(browserName) {
        return `
╔══════════════════════════════════════════════════════════════════════════════╗
║  Playwright browsers are not installed                                        ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
`;
    ...
    async getPage() {
        ...
        if (this.page) {
            try {
                ...
                if (this.config.verbose) {
                    console.log(`⚠️ Page unresponsive, recovering: ${e.message}`);
                }
                try {
                    await this.recoverBrowser();
                }
     ...
     */
    async detectCloudflareChallenge() {
        const page = await this.getPage();
        const evidence = [];
        let type = null;
        try {
            ...
            evidence.push(`Detection error: ${e.message}`);
        }
        return {
            detected: type !== null,
            type,
     ...
     */
    async waitForCloudflareResolution(timeout = 30000, checkInterval = 1000) {
        const page = await this.getPage();
        const startTime = Date.now();
    ...
    async click(selector, options = {}) {
        ...
        try {
            ...
            if (looksLikeUrl) {
                const ancestorAnchorInfo = await element.evaluate((el) => {
                    // If we're already an anchor, no change needed
                    if (el.tagName === 'A') {
                        return { isAnchor: true, href: el.href };
                    }
            ...
            const targetBlankRemoved = await clickTarget.evaluate((el) => {
            ...
            if (interception.intercepted && interception.interceptorSelector) {
                ...
                const handled = await this.handleStickyInterception(clickTarget, interception.interceptorSelector, { verbose: options.verbose });
                if (!handled.success) {
                    return {
                        success: false,
                        screenshot: await this.screenshot(),
     ...
     */
    async checkForInterception(element) {
        const page = await this.getPage();
        try {
     ...
     */
    async handleStickyInterception(element, interceptorSelector, options = {}) {
        const page = await this.getPage();
        // Strategy 1: Scroll element to viewport center (away from sticky headers/footers)
        try {
            if (options.verbose)
                console.log('  Trying strategy: scroll to safe zone');
        ...
        try {
            if (options.verbose)
                console.log('  Trying strategy: hide interceptor');
            const interceptor = page.locator(interceptorSelector).first();
            // Store original display value and hide
            const originalDisplay = await interceptor.evaluate((el) => {
            ...
            await interceptor.evaluate((el, original) => {
        ...
        try {
            if (options.verbose)
                console.log('  Trying strategy: force JS click');
            await element.evaluate((el) => {
                el.click();
            });
     ...
     */
    async hoverClick(selector, options = {}) {
        const page = await this.getPage();
        try {
            ...
            const possibleParents = [
                ...
                // Common menu words
                ...selectorWords.filter(w => ['Admissions', 'Academics', 'About', 'Resources', 'Campus', 'Student', 'Apply'].includes(w)),
            ];
    ...
    generateTestScenarios(analysis) {
            ...
            if (navLinks.length > 0) {
                tests.push({
                    ...
                    steps: navLinks.slice(0, 5).flatMap(link => [
                        { action: "navigate", target: analysis.url, description: "Start from home" },
                        { action: "click", target: link.selector, description: `Click ${link.text || "link"}` },
                        { action: "assert", target: `url contains '${new URL(link.href || "").pathname}'`, description: "Verify navigation" },
                    ]),
        ...
        const actionButtons = analysis.buttons.filter(b => b.text && !b.text.toLowerCase().includes("submit") && b.text.length < 30);
     ...
     */
    generateCBrowserScript(tests, url) {
     ...
     */
    generatePlaywrightCode(tests, url) {
     ...
     */
    async getAvailableClickables(page) {
        ...
        try {
            ...
            return await targetPage.$$eval('button, a, [role="button"], [role="link"], input[type="submit"], input[type="button"], [onclick], [type="submit"]', els => {
                ...
                return els
                    ...
                    text: el.innerText?.trim().substring(0, 60) || el.getAttribute("aria-label") || el.getAttribute("value") || "",
        ...
        try {
            return await activePage.$$eval('input, textarea, select', els => els.slice(0, 20).map(el => {
                ...
                if (input.id) {
                    ...
                    if (labelEl)
                        label = labelEl.innerText?.trim().substring(0, 50) || "";
                }
     ...
     */
    generateClickSuggestion(selector, available) {
     ...
     */
    generateFillSuggestion(selector, available) {
        if (available.length === 0) {
     ...
     */
    async captureDebugScreenshot(page, options) {
        try {
     ...
     */
    async analyzeFailure(selector, attempts) {
        const page = await this.getPage();
        try {
            // Get page context for suggestion
            const buttons = await page.$$eval('button, a, [role="button"]', els => els.slice(0, 10).map(el => ({
    ...
    async evaluateAssertion(assertion) {
        ...
        if (countMatch) {
            ...
            switch (elementType.replace(/s$/, "")) {
                case "button":
                    selectorForType = "button, [role='button']";
                    break;
                case "link":
                    selectorForType = "a";
                    break;
        ...
        try {
            ...
            for (const frame of frames) {
                if (frame === page.mainFrame())
                    continue; // Skip main frame, already searched
     ...
     */
    async findFillableElement(selector) {
        const page = await this.getPage();
    ...
    async extract(what) {
        ...
        switch (what.toLowerCase()) {
            case "links":
                data = await page.evaluate(() => Array.from(document.querySelectorAll("a")).map((a) => ({
                    text: a.textContent?.trim(),
                    href: a.href,
                })));
                break;
            case "headings":
                data = await page.evaluate(() => Array.from(document.querySelectorAll("h1, h2, h3, h4, h5, h6")).map((h) => ({
                    level: h.tagName,
                    text: h.textContent?.trim(),
                })));
                break;
            case "images":
                data = await page.evaluate(() => Array.from(document.querySelectorAll("img")).map((img) => ({
                    src: img.src,
                    alt: img.alt,
                })));
                break;
            case "forms":
                data = await page.evaluate(() => Array.from(document.querySelectorAll("form")).map((form) => ({
                    action: form.action,
                    method: form.method,
     ...
     */
    async screenshotCompressed(path, options) {
        const page = await this.getPage();
        ...
        for (const tryScale of scaleSteps) {
            for (const tryQuality of qualitySteps) {
                ...
                if (tryQuality < quality && tryScale >= scale)
                    continue;
        ...
        if (isApiKeyConfigured()) {
            try {
                ...
                return {
                    ...
                    frictionPoints: cognitiveResult.frictionPoints.map(fp => `${fp.type}: ${fp.monologue.substring(0, 80)}`),
                ...
                console.warn(`Cognitive journey failed, falling back to heuristic: ${e.message}`);
        ...
        while (stepCount < maxSteps && !success) {
            ...
            const clickable = await page.evaluate((goalText) => {
                ...
                const scored = elements.slice(0, 20).map((el) => {
                    ...
                    if (textLower.match(/submit|continue|next|proceed|sign|login|register/))
                        score += 1;
     ...
     */
    audit(action, target, zone, result) {
        const entry = {
            timestamp: new Date().toISOString(),
            action,
            target,
     ...
     */
    cleanup(options = {}) {
        const { dryRun = false, olderThan = 7, keepScreenshots = 10, keepJourneys = 5, keepSessions = 3, } = options;
        const result = {
            deleted: 0,
            freedBytes: 0,
            details: {
