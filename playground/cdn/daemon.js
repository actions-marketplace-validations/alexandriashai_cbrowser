...
import { getPaths, mergeConfig } from "./config.js";
...
export function getDaemonState() {
    ...
    try {
        ...
        console.debug(`[CBrowser] Daemon state file corrupted: ${e.message}`);
        return null;
    }
}
/**
...
export async function isDaemonRunning() {
    ...
    try {
        ...
        console.debug(`[CBrowser] Daemon not responding, cleaning up stale state: ${e.message}`);
        clearDaemonState();
        return false;
    }
}
 ...
 */
export async function sendToDaemon(command, args = {}) {
    const state = getDaemonState();
    if (!state) {
...
export async function startDaemon(port = DEFAULT_PORT) {
    ...
    const child = spawn(process.execPath, [process.argv[1], "daemon", "run", "--port", String(port)], {
        detached: true,
 ...
 */
export async function runDaemonServer(config, port = DEFAULT_PORT) {
    let browser = null;
    let idleTimer = null;
    ...
    const handleCommand = async (req) => {
        ...
        try {
            ...
            switch (req.command) {
                ...
                case "hoverClick": {
                    const selector = req.args.selector;
                    const hoverParent = req.args.hoverParent;
